  ...
  (
    {
      slideData,
      imageUrl,
      slideImageUrl: propSlideImageUrl,
      onAnnotationChange,
      initialPosition,
      tileName,
      Doctor = "Maharshi",
    ...
    const [tool, setTool] = useState<
      "pointer" | "move" | "rectangle" | "circle" | "ruler"
    >("pointer");
    ...
    const slideImageUrl =
      propSlideImageUrl || imageUrl || "/slides/histo_image.jpg";
    ...
    useEffect(() => {
      if (osdViewerRef.current && osdViewerRef.current.canvas) {
        osdViewerRef.current.canvas.style.filter = `brightness(${brightness}%) contrast(${contrast}%) saturate(${saturation}%)`;
      }
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 relative bg-background min-h-[400px]">
          ...
          <div
            ...
            style={{
              pointerEvents:
                tool !== "pointer" && tool !== "move" ? "auto" : "none",
              cursor:
          ...
          >
            ...
            {currentAnnotation && (
              <>
                ...
                {currentAnnotation.type === "measurement" &&
                  currentAnnotation.startPoint &&
                  currentAnnotation.endPoint && (
                    <svg className="absolute inset-0 w-full h-full pointer-events-none">
                      ...
                      />
                      <circle
                        cx={currentAnnotation.startPoint.x}
                        cy={currentAnnotation.startPoint.y}
                        r="4"
                        fill={currentAnnotation.color}
                      />
                      <circle
                        cx={currentAnnotation.endPoint.x}
                        cy={currentAnnotation.endPoint.y}
                        r="4"
                        fill={currentAnnotation.color}
                      />
                      {currentAnnotation.distance && (
                        <text
                          x={
                            (currentAnnotation.startPoint.x +
                              currentAnnotation.endPoint.x) /
                            2
                          }
                          y={
                            (currentAnnotation.startPoint.y +
                              currentAnnotation.endPoint.y) /
                              2 -
                            10
                          }
                          fill={currentAnnotation.color}
          ...
          <div className="absolute bottom-2 left-2 bg-black/75 text-white text-xs px-2 py-1 rounded z-20">
            ...
            {viewportInfo.y.toFixed(3)}) | Tool: {tool}
        ...
        <div className="flex items-center justify-between p-2 bg-muted border-t text-sm flex-wrap gap-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Badge variant="outline" className="bg-muted">
              {annotations.filter((a) => a.type === "measurement").length}{" "}
              Measurements
            </Badge>
          </div>
        </div>
